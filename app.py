...
# --- 기본 리더십 원칙 ---
DEFAULT_PRINCIPLES = """핵심 가치 3.1
이 가치를 지키며 일할 때, 훌륭한 팀원(high performer)들은 더 좋은 성과를 낼 수 있고 동료의 인정을 받을 수 있습니다. 또한 이 가치를 통해 시장에서 승리할 수 있고 혁신을 지속할 수 있습니다.
 
Mission over Individual 개인의 미션을 우선한다
팀원은 개인보다 소속 팀, 소속 팀보다는 토스팀의 미션 달성을 우선순위에 둔다. 팀에는 탁월한 동료들과 일하며 배우고, 그들의 존경을 얻고, 멋진 변화를 함께 만들고 시장을 혁신하는 일이 안정을 추구하는 것보다 중요한 사람들이 모여 있으며, 이들이 서로 같은 목표를 추구할 때 더 강력한 조직이 됨을 기억한다.
 
Aim Higher 더 높은 수준을 추구하라
팀에서 탁월한 업무 처리의 스킬은 주어진 일을 잘 수행하는 데 그치지 않고, 업무 퀄리티와 판단력, 서과이 새로운 기조을 마련하는 것이다.
 
Speak Openly 솔직하게 소통하라
팀원은 자신의 솔직한 의견을 표현하는 것에 거리낌이 없다. 피드백이 자신에게 향하더라도, 그것이 진심 어린 의도에서 나온 것이라면 흔쾌히 받아들인다.
 
Be Brave 두려워하지 말라
팀원은 실패에 대한 두려움으로 도전을 회피하지 않으며, 합리적 판단 하에 과감한 의사결정을 내린다.
 
Execute 실행하라
팀원은 실행력이 뛰어나다. 이론적인 완벽함만을 추구하기 보단, 빠르게 시도하고 학습한다.
 
Play as a Team 팀으로서 플레이하라
팀원은 팀의 성과가 곧 나의 성과라고 생각하며, 팀원들의 성공을 돕는 데 주저하지 않는다.
"""
